...
bool DecodeWeather(WiFiClient& json, String Type) {
  ...
  if (Type == "forecast") {
    ...
    if (getUnits() == "I") Convert_Readings_to_Imperial();
  }
  return true;
}
//#########################################################################################
...
bool obtain_wx_data(WiFiClient& client, const String& RequestType) {
  const String units = (getUnits() == "M" ? "metric" : "imperial");
  client.stop(); // close connection before sending a new request
  HTTPClient http;
  String uri = "/data/2.5/" + RequestType 
    + "?q=" + getCity() + "," + getCountry() 
    + "&APPID=" + getApiKey() 
    + "&mode=json&units=" + units 
    + "&lang=" + getLanguage();
  ...
  http.begin(client, getServer(), 80, uri);
